...
int PND_createwindowfrom(_THIS, SDL_Window * window, const void *data){return -1;}
...
PND_gl_createcontext(_THIS, SDL_Window * window)
{
    ...
    wdata->gles_surface = eglCreateWindowSurface( phdata->egl_display, wdata->gles_configs[wdata->gles_config], (NativeWindowType)0, NULL );
...
PND_gl_makecurrent(_THIS, SDL_Window * window, SDL_GLContext context)
{
    ...
    if ((window == NULL) && (context == NULL)) {
        status =
            eglMakeCurrent(phdata->egl_display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                           EGL_NO_CONTEXT);
        if (status != EGL_TRUE) {
